...
import com.blackview.base.app.BaseConstant
import com.blackview.base.event.LogoutTipsBean
import com.blackview.base.event.Message
import com.blackview.base.event.SingleLiveEvent
import com.blackview.base.kt.ktToastShow
import com.blackview.base.kt.show
import com.blackview.base.rxnetword.exception.BusinessException
import com.blackview.base.rxnetword.exception.MultiDevicesException
import com.blackview.base.rxnetword.exception.NotSaleManException
import com.blackview.base.rxnetword.exception.TokenTimeOutException
...
open class BaseViewModel : AndroidViewModel(Utils.getApp()), LifecycleObserver {
    ...
    private fun parseThrowable(
        ...
        when (throwable) {
            is NotSaleManException -> {
                LiveEventBus.get<String>(BaseConstant.EVENT_SALEMAN_EXCEPTION).post(throwable.message)
            ...
            is MultiDevicesException -> {
                LiveEventBus.get<LogoutTipsBean>(BaseConstant.EVENT_LOGOUT)
    ...
    private fun doLogoutParser(exception: rxhttp.wrapper.exception.ParseException) {
        if (exception.errorCode == BaseConstant.LOGOUT_STATUS_CODE.toString()) {//code 是-500 的时候，表示token 过时，或者被占线
            LiveEventBus.get<LogoutTipsBean>(BaseConstant.EVENT_LOGOUT).post(LogoutTipsBean(false, ""))
        }
        exception.show()
    }
