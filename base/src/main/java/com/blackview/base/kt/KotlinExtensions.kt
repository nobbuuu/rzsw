...
import com.blackview.base.app.BaseConstant.TAG
...
fun String.ktToastShowCenter() {
    if (isNotEmpty()) {
        val toast = Toast.makeText(Utils.getApp(), this, Toast.LENGTH_SHORT)
        toast.setGravity(Gravity.CENTER, 0, 0)
        toast.show()
    }
}

fun String.ktToastShowShort() {
    if (isNotEmpty()) {
...
private fun <T> handleNetworkException(throwable: T): String? {
    val stringId =
        if (throwable is UnknownHostException) { //网络异常
            if (!isNetworkConnected(Utils.getApp())) R.string.network_error else R.string.notify_no_network
